...
def checkRequireValueInDataFile(pathDataFile, columnName):
    ...
    try :
        with open(pathDataFile, encoding='utf-8') as csvfile:
            ...
            countLine = 1
            for row in reader:
                if row[columnName] == None or row[columnName] == 'None' or row[columnName] =='':
...
def addPrefixSuffixValueFixedFromDic(inputFile, outputFile, dicPrefix, dicSuffix):
    """
    ...
    :param dicSuffix: le dictionnaire contenant le nom des column sur lesquelles ajouter un suffix
    :type dicSuffix: dict['columName']='suffix'
            ...
            for k in dicPrefix:
                ...
                if not(value==None):
                    if not(value.lower()=='none' or value==''):
                        dicDataCur[k] = dicPrefix[k]+value
                                  
            for ks in dicSuffix:
                value = dicDataCur[ks]
                if not(value==None):
                    if not(value.lower()=='none' or value==''):
                        dicDataCur[ks] = value+dicSuffix[ks]
...
def getDistinctValueInColumn_test():
    datasPath = "C:/Users/Public/Documents/MSHS/workflow/Guarnido/03-metadatas/omeka/"
    datasFilename = "Guarnido_omkReadyPush.csv"
    
    datasPath = datasPath+datasFilename
    
    columnName = 'Linked in collection'
